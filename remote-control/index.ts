  ...
  pi.registerCommand("remote", {
    ...
    handler: async (args: string[], ctx: ExtensionContext) => {
      ...
      if (!subcommand) {
        ...
        const ok = await ensureDaemon(ctx); // <-- Start daemon
        if (!ok) {
          pi.sendMessage(
            {
              customType: "remote-status",
              content:
                "Daemon could not be started. Check logs or build daemon.js.",
              display: true,
            },
            { triggerTurn: false },
          );
        ...
        try {
          ...
          if (!resp.success) {
            if (ctx.hasUI) ctx.ui.notify(`❌ Daemon error: ${resp.error ?? "unknown"}`, "error");
            return;
          }
          ...
          if (data.peers && data.peers.length > 0) {
            ...
            for (const peer of data.peers) {
              ...
              lines.push(`- ${peer.host}:${peer.port} — ${status} (${peer.sessionCount} sessions)`);
            }
          }
          pi.sendMessage(
          ...
          if (ctx.hasUI) {
            ctx.ui.notify(
              `❌ Failed to get daemon status: ${err instanceof Error ? err.message : String(err)}`,
              "error",
            );
          }
        }
      ...
      if (subcommand === "add") {
        const host = args[1];
        if (!host) {
          if (ctx.hasUI) ctx.ui.notify("❌ Usage: /remote add <host>", "error");
          return;
        }
        ...
        try {
          ...
          if (resp.success) {
            if (ctx.hasUI) ctx.ui.notify(`🌐 Connected to peer: ${host}`, "info");
          } else {
            if (ctx.hasUI) ctx.ui.notify(`❌ Failed to connect to ${host}: ${resp.error ?? "unknown"}`, "error");
          }
        } catch (err) {
          if (ctx.hasUI) {
            ctx.ui.notify(
      ...
      if (subcommand === "remove") {
        const host = args[1];
        if (!host) {
          if (ctx.hasUI) ctx.ui.notify("❌ Usage: /remote remove <host>", "error");
          return;
        }
        try {
          const resp = await sendDaemonCommand({ type: "remove_peer", host }, 5000);
          if (resp.success) {
            if (ctx.hasUI) ctx.ui.notify(`Removed peer: ${host}`, "info");
          } else {
            if (ctx.hasUI) ctx.ui.notify(`❌ ${resp.error ?? "Failed to remove peer"}`, "error");
          }
        } catch (err) {
          if (ctx.hasUI) {
            ctx.ui.notify(
      ...
      if (subcommand === "list") {
        try {
          ...
          if (!resp.success) {
            if (ctx.hasUI) ctx.ui.notify(`❌ ${resp.error ?? "Failed to list sessions"}`, "error");
            return;
          }
      ...
      if (subcommand === "list-tailscale") {
        ...
        try {
          ...
          if (!resp.success) {
            if (ctx.hasUI) {
              ctx.ui.notify(
                `⚠️ Tailscale unavailable: ${resp.error ?? "unknown"}`,
                "warning",
              );
            }
            return;
      ...
      if (subcommand === "kill-daemon") {
        if (!fs.existsSync(DAEMON_SOCK)) {
          if (ctx.hasUI) ctx.ui.notify("Daemon is not running", "info");
          return;
        }
        try {
          await sendDaemonCommand({ type: "kill" }, 5000);
          if (ctx.hasUI) ctx.ui.notify("Daemon stopped", "info");
        } catch {
